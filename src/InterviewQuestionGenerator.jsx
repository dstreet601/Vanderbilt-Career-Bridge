...
const INDUSTRIES = [
  "Investment Banking","Consulting","Technology / Software","Healthcare",
  "Marketing & Advertising","Non-Profit / Government","Law","Real Estate",
  "Asset Management","General / Any Industry",
];
...
export default function InterviewQuestionGenerator() {
  ...
  const generateQuestions = async () => {
    ...
    const systemPrompt = "You are an expert career coach for Vanderbilt University students. Always return a valid JSON array and nothing else.";
    const userPrompt = "Generate 6 " + interviewType + " interview questions for a Vanderbilt student applying to a " + experienceLevel + " " + role + " role in the " + industry + " industry." + (orgContext ? " The student is involved in: " + orgContext + "." : "") + " Return ONLY a JSON array with exactly 6 objects each having: question (string), why (string), tip (string), example_opener (string). No markdown, no extra text.";
    try {
  ...
  const getFeedback = async (question) => {
    ...
    const systemPrompt = "You are an expert Vanderbilt career coach giving concise, actionable interview feedback.";
    const userPrompt = "Question: " + question + " Answer: " + userAnswer + " Give feedback in 3 sections: 1. Strengths (1-2 bullets) 2. Improve (1-2 bullets) 3. Revised opener (one better opening sentence). Under 200 words.";
    try {
      const res = await fetch("/api/claude", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ system: systemPrompt, messages: [{ role: "user", content: userPrompt }], max_tokens: 400 }) });
      if (!res.ok) throw new Error("API error");
      const data = await res.json();
      setFeedback((data && data.content && data.content[0] && data.content[0].text) || (data && data.text) || "");
    } catch (e) { setFeedback("Error getting feedback. Please try again."); }
  ...
  const inp = { width: "100%", padding: "10px 14px", borderRadius: 8, border: "1.5px solid #333", background: "#0d0d1a", color: "#fff", fontSize: 14, outline: "none", boxSizing: "border-box" };
      ...
      <div style={{ maxWidth: 780, margin: "0 auto" }}>
        ...
        <div style={{ background: CARD_BG, border: "1px solid #222", borderRadius: 14, padding: 24, marginBottom: 24 }}>
          <div style={{ marginBottom: 20 }}>
            ...
            <input style={inp} placeholder="e.g. Investment Banking Analyst, Product Manager..." value={role} onChange={(e) => setRole(e.target.value)} />
          </div>
          <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={lbl}>Student Org / Leadership <span style={{ color: "#555", fontWeight: 400 }}>(Optional)</span></label>
            <input style={inp} placeholder="e.g. VIC President, SGA Finance Chair..." value={orgContext} onChange={(e) => setOrgContext(e.target.value)} />
          </div>
          {error && <p style={{ color: "#f87171", fontSize: 13, marginBottom: 12 }}>{error}</p>}
          <button onClick={generateQuestions} disabled={loading} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", background: loading ? "#555" : GOLD, color: BLACK, fontSize: 15, fontWeight: 800, cursor: loading ? "not-allowed" : "pointer" }}>
        ...
        {practiceMode && activePracticeQ !== null && questions[activePracticeQ] && (
          <div style={{ background: "#0a0a18", border: "2px solid " + GOLD, borderRadius: 14, padding: 24, marginBottom: 24 }}>
            ...
            <textarea style={{ ...inp, minHeight: 120, resize: "vertical", lineHeight: 1.6 }} placeholder="Type your answer... Use STAR: Situation - Task - Action - Result" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} />
            <button onClick={() => getFeedback(questions[activePracticeQ].question)} disabled={feedbackLoading || !userAnswer.trim()} style={{ marginTop: 12, padding: "11px 24px", borderRadius: 8, border: "none", background: feedbackLoading || !userAnswer.trim() ? "#333" : GOLD, color: BLACK, fontSize: 14, fontWeight: 700, cursor: feedbackLoading || !userAnswer.trim() ? "not-allowed" : "pointer" }}>
        ...
        {questions.length > 0 && !practiceMode && (
          <div>
            ...
            {questions.map((q, idx) => (
              <div key={idx} style={{ background: CARD_BG, border: "1px solid " + (expandedIdx === idx ? GOLD : "#222"), borderRadius: 12, padding: "18px 20px", marginBottom: 12, cursor: "pointer" }} onClick={() => setExpandedIdx(expandedIdx === idx ? null : idx)}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                  ...
                  <span style={{ color: "#555", fontSize: 18 }}>{expandedIdx === idx ? "^" : "v"}</span>
                </div>
                {expandedIdx === idx && (
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #222" }} onClick={(e) => e.stopPropagation()}>
                    ...
                    <button onClick={() => startPractice(idx)} style={{ padding: "9px 20px", borderRadius: 8, border: "1.5px solid " + GOLD, background: "transparent", color: GOLD, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
